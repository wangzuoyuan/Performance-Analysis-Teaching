...
根目录 `.env` 默认固定 `IMAGE_TAG=2.0.5`，比 `latest` 更容易审计和回滚。然后复制 `backend/.env.example` 为 `backend/.env`，填：
...
```bash
docker pull ghcr.io/wangzuoyuan/performance-analysis-teaching-backend:2.0.5
docker pull ghcr.io/wangzuoyuan/performance-analysis-teaching-frontend:2.0.5
docker save \
  ghcr.io/wangzuoyuan/performance-analysis-teaching-backend:2.0.5 \
  ghcr.io/wangzuoyuan/performance-analysis-teaching-frontend:2.0.5 \
  -o exam-images-2.0.5.tar
